...
class parameters():
    ...
    # 'method' is unused for spin codes at the moment. Call them separately in sbatch script.
...
def Hel(x):
    ...
    D =  np.array([0.003,  0.004,  0.003])
    b =  np.array([0.650,  0.600,  0.650])
    Re = np.array([5.000,  4.000,  6.000])
    c =  np.array([0.000,  0.010,  0.006])
    
    A =  np.array([0.002,  0.002])
    Rcross =  np.array([3.400,  4.800])
    a =  np.array([16.00,  16.00])
    ...
    Hel[0,1] = A[0] * np.exp( -a[0] * (x - Rcross[0])**2 )
    Hel[1,2] = A[1] * np.exp( -a[1] * (x - Rcross[1])**2 )
...
def dHel(x):
    ...
    dHel[0,1,0] = -2 * a[0] * A[0] * np.exp( -a[0] * (x - Rcross[0])**2 ) * ( x - Rcross[0] )
    dHel[1,2,0] = -2 * a[1] * A[1] * np.exp( -a[1] * (x - Rcross[1])**2 ) * ( x - Rcross[1] )

    dHel[1,0,0] = dHel[0,1,0]
    dHel[2,1,0] = dHel[1,2,0]
